...
#define LEAKY_RELU_ALPHA (0.01f)//a

static inline void hvx_leaky_relu_f32_inner(float *restrict dst, const float *restrict src, int ne0) {
  const int n_vecs = ne0 / 32;

  const HVX_Vector *pv_in   = (const HVX_Vector *) src;
  HVX_Vector       *pv_out  = (HVX_Vector *) dst;
  const HVX_Vector  v_zero  = Q6_V_vzero();
  const HVX_Vector  v_alpha = Q6_V_vsplat_R(*(const int32_t *) &(float){ LEAKY_RELU_ALPHA });
  ...
  for (int i = 0; i < n_vecs; ++i) {
    ...
    HVX_Vector     v_neg    = Q6_Vsf_equals_Vqf32(Q6_Vqf32_vmpy_VsfVsf(v_x, v_alpha));
    HVX_VectorPred q_pos    = Q6_Q_vcmp_gt_VsfVsf(v_x, v_zero);
    *pv_out++               = Q6_V_vmux_QVV(q_pos, v_x, v_neg);
  }

  const int n_done = n_vecs * 32;
  for (int i = n_done; i < ne0; ++i) {
    dst[i] = src[i] > 0.0f ? src[i] : (LEAKY_RELU_ALPHA * src[i]);
  }
}
...
int hvx_leaky_relu_f32(float *restrict dst, const float *restrict src, int ne0, int ne1) {
  ...
  for (int j = 0; j < ne1; ++j) {
    ...
    hvx_leaky_relu_f32_inner(out_row, in_row, ne0);
  }
