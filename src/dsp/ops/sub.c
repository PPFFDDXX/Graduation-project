...
static inline void hvx_sub_f32_inner(float *restrict dst, const float *restrict src0, const float *restrict src1,
                                     int ne0) {
  ...
  for (int i = 0; i < n_vecs; ++i) {
    ...
    *pv_out++ = Q6_Vqf32_vsub_VsfVsf(*pv_in0++, *pv_in1++);
  }
...
int hvx_sub_f32(float *restrict dst, const float *restrict src0, const float *restrict src1, int ne0, int ne1) {
  ...
  for (int j = 0; j < ne1; ++j) {
    ...
    hvx_sub_f32_inner(out_row, src0_row, src1_row, ne0);
  }
