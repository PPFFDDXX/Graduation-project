...
static inline void hvx_mpy_f32_inner(float *restrict dst, const float *restrict src0, const float *restrict src1,
                                     int ne0) {
...
int hvx_mpy_f32(float *restrict dst, const float *restrict src0, const float *restrict src1, int ne0, int ne1) {
  ...
  for (int j = 0; j < ne1; ++j) {
    ...
    hvx_mpy_f32_inner(out_row, src0_row, src1_row, ne0);
  }
